...
# Note: 'mise' is intentionally excluded from the plugins list here and activated manually below
# to use '--no-hook-env' which skips the initial _mise_hook() call at startup (saves ~17ms).
# The _mise_hook_precmd hook still runs before the first prompt, so tool versions are set correctly.
plugins=(direnv eza fast-syntax-highlighting git iterm2 sudo zbell zsh-autosuggestions)
...
# Activate mise with --no-hook-env to skip the initial _mise_hook() run at startup (saves ~17ms).
# _mise_hook_precmd will still fire before the first prompt, ensuring tool versions are correct.
command_exists mise && eval "$(mise activate zsh --no-hook-env)"
